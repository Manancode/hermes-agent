...
class MemoryTencentdbProvider(MemoryProvider):
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        Resolution order: kwargs > env vars > defaults.
        agent_identity (Hermes profile name) is used only for logging, NOT as TencentDB agent_id.
        """
        # Ensure .env is loaded — long-lived gateway processes may have been
        # started before dotenv injection or under a profile scope that
        # deferred the process-global load.
        if not os.environ.get("MEMORY_TENCENTDB_GATEWAY_HOST"):
            try:
        ...
        self._user_id = kwargs.get("user_id") or os.environ.get("MEMORY_TENCENTDB_USER_ID") or _DEFAULT_USER_ID
        self._team_id = kwargs.get("team_id") or os.environ.get("MEMORY_TENCENTDB_TEAM_ID") or _DEFAULT_TEAM_ID
        self._agent_id = kwargs.get("agent_id") or os.environ.get("MEMORY_TENCENTDB_AGENT_ID") or _DEFAULT_AGENT_ID
        self._profile_name = kwargs.get("agent_identity", "unknown")
    ...
    def handle_tool_call(self, tool_name: str, args: Dict[str, Any], **kwargs) -> str:
        ...
        try:
            ...
            if tool_name == "memory_tencentdb_conversation_search":
                ...
                self._record_success()
                items = result.get("data", {}).get("messages", result.get("data", {}).get("items", []))
                if not items:
